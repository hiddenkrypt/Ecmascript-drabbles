...
function start() {
	...
	plant = true;
	snake.bod.push({
		x : Math.floor(gridMax / 2),
		y : Math.floor(gridMax / 2)
	});
	snake.bod.push({
		x : Math.floor(gridMax / 2) + 1,
		y : Math.floor(gridMax / 2)
	});
	snake.bod.push({
		x : Math.floor(gridMax / 2) + 2,
		y : Math.floor(gridMax / 2)
	});
	snake.bod.push({
		x : Math.floor(gridMax / 2) + 3,
		y : Math.floor(gridMax / 2)
	});
}
(function () {
...
function spawnFood() {
	if (plant && Math.random() < foodChance && food.length < gridMax * gridMax) {
		if (food.length >= gridMax * gridMax * 0.9) {
			plant = false;
		}
		food.push({
			x : Math.floor(Math.random() * gridMax),
